...
//            metric = args[1];
        ...
        double[] taus = new double[]{0.6, 0.5, 0.4};
        int[] maxPLefts = new int[]{1, 2};
        int[] maxPRights = new int[]{2, 3, 4};
        int topK = 10_000_000;
//        int[] topKs =  new int[]{10,25,50,100,500,1000};
        double[] minJumps = new double[]{0.2, 0.15, 0.1};
...
//                for(int topK : topKs) {
//                    if (maxPLeft + maxPRight > 5) continue; // only check cardinality <=4 for paper experiment
//                    if(n_vec > 1000 && maxPLeft + maxPRight > 4) continue; // max cardinality 4 for the sotck data
//                    for (double minJump : minJumps) {
//                        System.gc();
//
//                        configurationParameters params = new configurationParameters(data, metric, tau, minJump, start_epsilon,
...
//        minJumps = new double[]{-5., 0.05, 0.1, 0.15};
        ...
        for(int maxPLeft : maxPLefts){
            for(double tau : taus){
                for(double minJump : minJumps){
//                    if(n_vec > 1000 && maxPLeft > 4) continue; // max cardinality 4 for the sotck data
                    System.gc();
