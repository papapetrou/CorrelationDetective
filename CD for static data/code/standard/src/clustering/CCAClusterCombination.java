...
public class CCAClusterCombination extends ClusterCombination {
    ArrayList<Cluster> LHS;
    ArrayList<Cluster> RHS;

    //TODO: CCA is not functional yet, the bounds are probably incorrect



    public CCAClusterCombination(ArrayList<Cluster> LHS, ArrayList<Cluster> RHS){
        super();
        this.LHS = LHS; this.RHS = RHS;
        this.isCCACandidate = true;
    ...
    public void swapLeftRightSide(){
        ...
        //todo: we are forgetting about pairwise correlations within groups and also about smaller sized subsets than |canidadate|-1
        double subsetcorr;
        double max=-1;
        ...
        if(LHS.size() > 1){
            for(int i=0; i<LHS.size(); i++){
                ...
                if(Math.abs(subsetcorr - cc.getUB()) > 0.001){
                    System.err.println("MultipoleClusterCombination.java: this should only be called on clusters of size 1");
                }
    ...
    public ArrayList<ClusterCombination> getSplittedCC(){
        ...
        if(cToBreak < LHS.size()){ // the largest radius is in the left hand side
            ...
            for(Cluster sc : largest.getSubClusters()){
                ...
                CCAClusterCombination newCandidate = new CCAClusterCombination(new ArrayList<>(newLHS), new ArrayList<>(RHS));
            ...
            for(Cluster sc : subclusters){
                ...
                // this copying is necessary as otherwise we'll remove the cToBreak IN PLACE within the newCandidate object
                CCAClusterCombination newCandidate = new CCAClusterCombination(new ArrayList<>(LHS), new ArrayList<>(newRHS));
                newCombinations.add(newCandidate);
