...
//
        ...
        double maxApproximationSize = Math.sqrt(n_dim * (1- (0)));
//        maxApproximationSize = Double.MAX_VALUE;


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        configurationParameters params = new configurationParameters(fmri, metric, tau, minJump,  start_epsilon, epsilon_multiplier,
...
//
    ...
    public static void run(configurationParameters par, boolean printResults){
        ...
        System.out.println("parameters -- tau (correlation threshold) = " + par.tau  + "; minJump = " + par.minJump + "; start epsilon = " + par.start_epsilon +
...
//
        ...
        AprioriPruning AP = new AprioriPruning(HB, par.maxPLeft, par.tau, par.useEmpiricalBounds, par.minJump,
        par.parallelEvaluation, par.vecNames);
        ...
        positiveDCCs = AP.positiveDCCs;
         results = AP.positiveResultSet;
            ...
            HB.recursiveBounding(par.start_epsilon, par.tau, par.epsilon_multiplier, par.useEmpiricalBounds, par.minJump,
                    par.shrinkFactor, par.maxApproximationSize, par.numberOfPriorityBuckets, par.topK,
...
//         print the results if desired:
        if(printResults){
            ...
            PostProcessResults.printResultSet(positiveDCCs, CB, par.minJump, par.vecNames, par.parallelEvaluation);
        }
    ...
    public static double[][] loadDataWeather(String filename, int desired_stocks, int n_dim) {
        ...
        double[][] dataArray = readWeatherData.readWeather(filename, desired_stocks, n_dim, Measure.Attribute.temp);
...
//        Collections.shuffle(data, new Random(123));
    ...
    static double[][] loadData(String fileLoc, int desired_vecs, int desired_dim, ArrayList<String> header){
        ...
        try{
            ...
            System.out.println("no csv file, trying weather methods...");
        }
        ...
        try{
            ...
            System.out.println("weather didn't work out either... Last hope is stock methods..");
        }
