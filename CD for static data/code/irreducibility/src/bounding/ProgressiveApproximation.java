...
//    public ArrayList<ClusterCombination> additionalPositives = new ArrayList<>(1000);
    ...
    public ProgressiveApproximation(double corrThreshold, boolean useEmpriricalBounds, double maxApproximationSize, List<String> header, CorrelationBounding CB){
...
//
            Map<Integer, List<ClusterCombination>> processedDCCs = priorityDCCStream.unordered() // all priority buckets that come from here should have lower priority ie higher id then the key currently being processed
            .flatMap(cc -> (HierarchicalBounding.corrBounding(cc, corrThreshold, useEmpiricalBounds, parallel, 1, maxApproximationSize)).stream())
            .collect(Collectors.groupingBy(cc -> getPriorityBucket(cc.getCriticalShrinkFactor(), numberOfPriorityBuckets)));
            ...
            positiveDCCs.addAll(PostProcessResults.unpackAndCheckNoSuperset(partialAdditionalPositives, CB, corrThreshold, parallel));
