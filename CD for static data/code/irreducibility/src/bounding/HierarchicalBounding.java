...
//        this.n_2corr_lookups = 0;
    ...
    public void recursiveBounding(double startEpsilon, double corrThreshold, double epsilonMultiplier, boolean useEmpiricalBounds,
                                  double shrinkFactor, double maxApproximationSize, int numBuckets, int topK,
        ...
        PA = new ProgressiveApproximation(corrThreshold, useEmpiricalBounds, maxApproximationSize, header, CB);
        ...
        List<ClusterCombination> pairwiseDCCs = corrBounding(PairwiseRootCandidate, corrThreshold, useEmpiricalBounds, parallel, 1, maxApproximationSize);
        ...
        this.positiveDCCs.addAll(PostProcessResults.unpackAndCheckNoSuperset(pairwisePostives, CB, corrThreshold, parallel));
...
//         now start with multivariate correlations //////////////////////////
        ...
        DCCs = getStream(rootCandidateList, parallel).unordered()
                .flatMap(cc -> getStream(corrBounding(cc, finalCorrThreshold, useEmpiricalBounds, parallel, shrinkFactor, maxApproximationSize), parallel))
...
//                .collect(Collectors.partitioningBy(ClusterCombination::isPositive));
        ...
        this.positiveDCCs.addAll(PostProcessResults.unpackAndCheckNoSuperset(DCCs.get(true), CB, corrThreshold, parallel));
        if(positiveDCCs.size() > topK){
            positiveDCCs = getStream(positiveDCCs, parallel)
                    .sorted((cc1, cc2) -> Double.compare(cc2.getLB(), cc1.getLB()))
                    .limit(topK)
        ...
        this.positiveDCCs = PostProcessResults.unpackAndCheckNoSuperset(this.positiveDCCs, CB, corrThreshold, parallel);
    ...
    public static List<ClusterCombination> corrBounding(ClusterCombination CC, double corrThreshold, boolean useEmpiricalBounds, boolean parallel, double shrinkFactor, double maxApproximationSize){
        ...
        if (true) {
            ...
            if ((LB < corrThreshold) && (shrunkUB >= corrThreshold) &&(CC.getMaxLowerBoundSubset() < corrThreshold)) { // we need more precise clusters to be conclusive, it is possible that we are highly correlated
                ...
                return getStream(newCandidates, parallel).unordered()
                        .flatMap(newCC -> corrBounding(newCC, corrThreshold, useEmpiricalBounds, parallel, shrinkFactor, maxApproximationSize).stream())
                        .collect(Collectors.toList());
...
//                this.n_decided += CC.getNComparisons();
                ...
                if(shrunkUB < corrThreshold || CC.getMaxLowerBoundSubset() >= corrThreshold) { // negative DCC
...
//                    }
                    ...
                    if(CC.getSlack() > 0 && corrThreshold <= 1){ //
                        criticalShrinkFactor = (corrThreshold - CC.getCenterOfBounds())/CC.getSlack();
                        if(criticalShrinkFactor < shrinkFactor && CC.getMaxLowerBoundSubset() < corrThreshold){
                    ...
                    if(criticalShrinkFactor <= 1 && CC.getMaxLowerBoundSubset() < corrThreshold){ // don't bother further investigation if subset > tau or positive is impossible
                        out.add(CC);
                    }
                }else if(LB >=corrThreshold){ //positive DCC
